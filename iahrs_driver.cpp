...
typedef struct IMU_DATA
{
    ...
    double dMagnetic_field_x = 0.0;
    double dMagnetic_field_y = 0.0;
    double dMagnetic_field_z = 0.0;
...
int main(int argc, char** argv)
{
    ...
    ros::Publisher mag_data_pub = nh.advertise<sensor_msgs::MagneticField>("imu/mag", 1);
    ...
    while(ros::ok())
    {
        ...
        if (serial_fd >= 0) 
        {
            ...
            // Read angular_velocity (wx, wy, wz)
            no_data = SendRecv("g\n", data, max_data);
            if (no_data >= 3) 
            {
            ...
            // Read linear_acceleration (unit: m/s^2)
            no_data = SendRecv("a\n", data, max_data);
            if (no_data >= 3) 
            {
            ...
            no_data = SendRecv("m\n", data, max_data); // 'm\n' 명령어는 가상의 명령어로 실제 명령어로 교체 필요
            if (no_data >= 3)
{
    _pIMU_data.dMagnetic_field_x = data[0];
    _pIMU_data.dMagnetic_field_y = data[1];
    _pIMU_data.dMagnetic_field_z = data[2];

    ROS_INFO("Magnetic Field Data: X: %f, Y: %f, Z: %f", _pIMU_data.dMagnetic_field_x, _pIMU_data.dMagnetic_field_y, _pIMU_data.dMagnetic_field_z);

    // Publish Magnetic Field data
    sensor_msgs::MagneticField mag_data_msg;
    mag_data_msg.magnetic_field.x = _pIMU_data.dMagnetic_field_x;
    mag_data_msg.magnetic_field.y = _pIMU_data.dMagnetic_field_y;
    mag_data_msg.magnetic_field.z = _pIMU_data.dMagnetic_field_z;

    mag_data_msg.header.stamp = ros::Time::now();
    mag_data_msg.header.frame_id = tf_prefix_ + "/imu_link";
    mag_data_pub.publish(mag_data_msg);

    ROS_INFO("Magnetic Field data published");
}
else
{
    ROS_WARN("No magnetic data received");
}
